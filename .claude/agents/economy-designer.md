...
```bash
curl -s "${MEMORY_URL}/api/search?q=economy+loot+progression+resource+balance&namespace=patterns&top_k=5"
```

At END — save domain-specific insights:
```bash
curl -s -X POST ${MEMORY_URL}/api/memories   -H "Content-Type: application/json"   -d '{"text":"INSIGHT","namespace":"patterns","metadata":{"project":"smash-karts-clone","source":"economy-designer"}}'
```
