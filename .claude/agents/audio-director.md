...
```bash
curl -s "${MEMORY_URL}/api/search?q=audio+sound+music+direction+mix+balance&namespace=conventions&top_k=5"
```

At END — save domain-specific insights:
```bash
curl -s -X POST ${MEMORY_URL}/api/memories   -H "Content-Type: application/json"   -d '{"text":"INSIGHT","namespace":"conventions","metadata":{"project":"smash-karts-clone","source":"audio-director"}}'
```
