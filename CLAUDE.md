...
| Kart Physics | `kart-physics.md` | **v2.3 implemented, testing pending** (branch: `arcade-physics`). Continuous `intensity_target = pow(\|steer\|, exponent) * speed_factor` replaces binary hysteresis; `DRIFT_STEER_EXPONENT=3.0` new knob; thresholds removed; floor-align yaw-lock (pitch/roll only). v2.1, v2.2 archived. |
