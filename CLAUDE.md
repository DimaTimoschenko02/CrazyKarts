...
- **Визуал и физика — приоритет.** Поведение машинки, ощущение от управления, анимации взрывов — близко к оригинальному SmashKarts.io.
...
- **Изменение GDD:** Любые правки `design/gdd/*` — сначала `/design-review` или `systems-designer`, потом правки.
...
IMPLEMENTATION (текущая фаза):
  ...
  Агенты напрямую — ТОЛЬКО если нет подходящего скилла. Сначала ВСЕГДА проверь скиллы.
...
6. **Feel first** — при любом выборе приоритет ощущениям (из memory).
...
**Phase**: Design complete → Starting implementation
**Что работает сейчас**: базовый мультиплеер (lobby → game → rockets → kill/respawn)
**Что сломано**: HTML5 export (серый экран), explosion VFX (stub), drift feel
**Что нужно**: рефакторинг по GDD спецификациям, затем новые фичи
