...
| `DRIFTING` | Auto-drift active (steer+speed thresholds met), rear slide active | No | — |
...
| DRIVING | DRIFTING | `|steer_input| > drift_enter_threshold` AND `fwd_speed > drift_min_speed_ratio * max_speed` (auto-drift, no key) | Client request |
| DRIVING | DEAD | Damage event, HP <= 0 | Server only |
| DRIFTING | DRIVING | `|steer_input| < drift_exit_threshold` OR `fwd_speed <= drift_min_speed_ratio * max_speed` (hysteresis gap) | Client request |
...
| `drift_min_speed_ratio` | 0.4 | 0.2-0.6 | Min speed fraction for drift entry | Drift at near-standstill | Can only drift at 60%+ top speed |
